...
public sealed class S3PackageUpdaterBehaviorTests
{
    ...
    public async Task UpdateAsync_PrunesStaleTemporaryFilesBeforeInstallingAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths launcherPaths = TestLauncherPaths.Create(testDirectory);
        PackageUpdatePathSet canonicalPaths = CreatePackagePaths(launcherPaths);
        var paths = new PackageUpdatePathSet(
            new OwnedContentPath(
                launcherPaths.PackagesDirectory,
                Path.Combine(launcherPaths.PackagesDirectory, "NProject Mod", "2.11")),
            canonicalPaths.InstalledPath,
            canonicalPaths.BackupPath);
        ...
        string? latestRelativePath = null)
    {
        var installedPath = new OwnedContentPath(
            ...
            Path.Combine(launcherPaths.ModsDirectory, "installed"));
        OwnedContentPath? latestInstalledPath = latestRelativePath is null
            ? null
            : new OwnedContentPath(
                launcherPaths.ModsDirectory,
                Path.Combine(launcherPaths.ModsDirectory, latestRelativePath));
        return PackageUpdatePathSet.Create(launcherPaths, installedPath, latestInstalledPath);
    }
