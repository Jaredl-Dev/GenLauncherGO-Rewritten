...
internal sealed class LauncherExecutableManagementViewModel : ObservableObject
{
    ...
    public bool TrySaveEditor()
    {
        ...
        IReadOnlyList<LauncherCustomExecutable> currentEntries = gamePreferences.GetCustomExecutables(_kind);
        IReadOnlyList<LauncherCustomExecutable> updatedEntries = _editingExecutablePath == null
            ...
            : currentEntries.Select(entry => ExecutablePathsEqual(
                ...
                ? executable
                : entry).ToArray();
        LauncherGamePreferences updatedGamePreferences = gamePreferences.WithCustomExecutables(_kind, updatedEntries);
        if (_editingExecutablePath != null &&
            ExecutablePathsEqual(gamePreferences.GetSelectedExecutable(_kind), _editingExecutablePath))
        {
            updatedGamePreferences = updatedGamePreferences.WithSelectedExecutable(_kind, executable.ExecutablePath);
        }
    ...
    public bool Remove(ExecutableOption option)
    {
        ...
        LauncherPreferences current = _preferencesService.Current;
        LauncherGamePreferences gamePreferences = GetActiveGamePreferences(current);
        IReadOnlyList<LauncherCustomExecutable> updatedEntries = gamePreferences.GetCustomExecutables(_kind)
            .Where(executable => !ExecutablePathsEqual(executable.ExecutablePath, option.ExecutablePath))
            .ToArray();
        LauncherGamePreferences updatedGamePreferences = gamePreferences.WithCustomExecutables(_kind, updatedEntries);
        if (ExecutablePathsEqual(updatedGamePreferences.GetSelectedExecutable(_kind), option.ExecutablePath))
        {
            ...
            ExecutableOption? fallback = LauncherExecutableSelectionService.SelectPreferredOption(
                remainingOptions,
                null);
            updatedGamePreferences = updatedGamePreferences.WithSelectedExecutable(
                _kind,
