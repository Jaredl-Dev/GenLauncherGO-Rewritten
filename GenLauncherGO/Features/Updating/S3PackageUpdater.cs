...
internal sealed class S3PackageUpdater : IS3PackageUpdater
{
        ...
        CancellationToken cancellationToken)
    {
        List<S3DownloadWorkItem> downloadWorkItems = await CreateDownloadWorkItemsAsync(
            ...
            cancellationToken).ConfigureAwait(false);
        // Progress is measured against the whole package, not just what is left to fetch. Otherwise a resumed
        // download reports a shrinking total and a bar that restarts at zero, both of which read as lost work.
        long totalPackageSize = files.Sum(file => (long)file.Size);
        long remainingDownloadSize = downloadWorkItems.Sum(download => download.BytesToDownload);
        long resumedBytes = Math.Max(0, totalPackageSize - remainingDownloadSize);
        var progressState = new PackageProgressTracker(totalPackageSize, resumedBytes);
        if (downloadWorkItems.Count == 0)
        {
            progress?.Report(new PackageUpdateProgress(0, 0, 100, null));
        }

        SemaphoreSlim downloadSlots = new(MaxConcurrentFileDownloads);
    ...
    private async Task<List<S3DownloadWorkItem>> CreateDownloadWorkItemsAsync(
        ...
        CancellationToken cancellationToken)
    {
        List<S3DownloadWorkItem> downloads = [];
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 1; attempt <= MaxHashAttempts; attempt++)
        {
            ...
            {
                throw new IOException("Downloaded file validation failed after repeated download attempts.");
            }
            ...
            _logger.LogWarning(
                "Downloaded file validation failed for {FileName}; retrying download attempt {NextAttempt}.",
                file.FileName,
                attempt + 1);
            DeleteFailedFile(destinationFilePath);
    ...
    private static async Task<Uri> BuildDownloadUriAsync(
        IMinioClient client,
        string bucketName,
        string folderName,
        string fileName)
        ...
        string destinationFilePath)
    {
        ...
        if (!LexicalPath.AreEquivalent(packageRoot, destinationDirectory))
        {
        ...
        IReadOnlyList<RemoteFileManifestEntry> files)
    {
        ...
        foreach (RemoteFileManifestEntry file in files)
        {
            string destinationPath = ManifestPathResolver.ResolveInstalledPath(packageRoot, file.FileName);
