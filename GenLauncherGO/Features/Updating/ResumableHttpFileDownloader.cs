...
internal sealed class ResumableHttpFileDownloader : IResumableFileDownloader
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 1; attempt <= _maxAttempts; attempt++)
        {
            ...
            catch (Exception exception) when (IsRetriable(exception) && attempt < _maxAttempts)
            {
                ...
                _logger.LogWarning(
                    "Download attempt {Attempt} failed for {FileName}; failure type: {FailureType}; retrying.",
                    attempt,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.Resume &&
            request.ExpectedBytes.HasValue &&
            File.Exists(request.DestinationFilePath) &&
            new FileInfo(request.DestinationFilePath).Length == request.ExpectedBytes.Value)
        {
        ...
        if (existingBytes > 0)
        {
            ...
            if (request.IfRangeEntityTag is { IsWeak: false } entityTag)
            {
        ...
        bool serverAcceptedResume = existingBytes > 0 &&
                                    ...
                                    (request.IfRangeEntityTag is null ||
                                     response.Headers.ETag is null ||
                                     request.IfRangeEntityTag.Equals(response.Headers.ETag));
